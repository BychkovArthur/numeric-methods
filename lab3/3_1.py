...
def M(x):  # Для рассчета ошибки (n+1) производная
    return -math.sin(x)
...
def lagrange(xs, x):
    '''
        ...
        В этом случае, функция выглядит как:
        sum(i=0, n) (f_i * l_i), где f_i - значение в i-й точке (которое дано нам)
    '''
    n = len(xs)
    res = 0
...
def newton(xs, x):
    '''
        ...
        При добавлении новой точки можно не пересчитывать
    '''
    n = len(xs)
